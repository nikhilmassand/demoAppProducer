...
class AtomicKafka {
	...
	socketProduce (data, topic) {
		const localProducer = this.Producers[topic];
		localProducer.produce(data, topic);
	}

	socketSubmitProduce (topic) {
		const localProducer = this.SubmitProducers[topic];
		this.io.on('connection', (socket) => {
			socket.on('postMessage', (arg) => {
				localProducer.produce(arg, topic);
			})
		})
	}
}

// let newAK = new AtomicKafka('');
// newAK.newProducer('topic')
// const produceThisWay = () => {
// 	//file system
// 	//loop through data from file system
// 	//set interval
// 		//return a single piece of data ({payload: {data[i]}})
// }

// const x = produceThisWay();

// newAK.socketProduce('topic', produceThisWay) //pass in in
// newAK.socketProduce('topic', type = click/interval/custom, interval = "", cb) //pass in in 'postInterval' 'postMessage'
